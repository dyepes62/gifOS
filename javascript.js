...
function hoyTeRecomendamos (){
    let datosBusqueda= fetch('http://api.giphy.com/v1/gifs/trending?' + '&api_key='+ apiKey+ '&limit=4&rating=G')
    .then(response => {
...
function getTendencias(){
    let datosBusqueda= fetch('http://api.giphy.com/v1/gifs/trending?' + '&api_key='+ apiKey+ '&offset=4&limit=17&rating=G')
    .then(response => {
        return response.json();
    })
    .then(data =>{
        ...
        let div1 = document.getElementById("tendencia1");
        let barra= document.createElement("h2");
        div1.appendChild(barra);
        barra.className= "barraPrimeraTendencia";
        barra.innerHTML= "#wink #puppie #cute #hello #hi";
    })
    .catch(error => {
        return error;
    });
...
function getBusquedaResults(search){
    ...
    let datosBusqueda= fetch('http://api.giphy.com/v1/gifs/search?q='+ search + '&api_key='+ apiKey + '&offset=4&limit=17&rating=G')
    .then(response => {
        return response.json();
    })
    .then(data => {
...
let inputBuscar= document.querySelector("#inputBusqueda");
 ...
 function cambiosInput(){       
   ...
   if (valorinput !== ""){
        ...
        let datosBusqueda= fetch('http://api.giphy.com/v1/gifs/trending?' + '&api_key='+ apiKey+ '&limit=3&rating=G')
        .then(response => {
            return response.json();
        })
        .then(data =>{
